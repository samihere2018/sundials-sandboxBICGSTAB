  ...
  case 0:
    ...
    for (k = 0; k < n; k++)
    {
      ...
      else if (SUNRsqrt((sunrealtype)(conj(temp2) * temp2)) >= SUNRsqrt((sunrealtype)(conj(temp1) * temp1)))
      {
        ...
        // c     = -s * temp3; 
        denomCS = SUNRsqrt((sunrealtype)(conj(temp1)*temp1)+(sunrealtype)(conj(temp2)*temp2));//Amihere (use this and next two lines)
        c = conj(temp1)/denomCS;
        s = -conj(temp2)/denomCS;
        // c  = SUNRsqrt((sunrealtype)(conj(temp1)*temp1))/denomCS; 
        // signf = (1.0/SUNRsqrt((sunrealtype)(conj(c)*c)));
      ...
      else
      {
        ...
        c = conj(temp1)/denomCS;
        s = -conj(temp2)/denomCS;
        // c  = SUNRsqrt((sunrealtype)(conj(temp1)*temp1))/denomCS; //Amihere
        // signf = (1.0/SUNRsqrt((sunrealtype)(conj(c)*c)));
        // s  = -signf *conj(temp2)/denomCS; //Amihere
        // s = c * temp3;
